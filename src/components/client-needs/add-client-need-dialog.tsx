...
export function AddClientNeedDialog({
  productId,
  availableCatalog,
  allProducts,
}: {
  ...
  const sorted = [...availableCatalog].sort((a, b) => a.itemName.localeCompare(b.itemName));
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size="sm">+ Add Client Need</Button>
      </DialogTrigger>
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-lg">
        ...
        <Tabs defaultValue="list">
          ...
          <TabsContent value="list" className="space-y-3 pt-2">
            ...
            <form ref={formRef} onSubmit={handleAttach} className="space-y-3">
              <div className="max-h-72 space-y-1 overflow-y-auto rounded-md border p-2">
                ...
                {sorted.length === 0 && (
                  <p className="px-2 py-4 text-center text-sm text-muted-foreground">
                    Nothing left to add — every catalog item is already on this product.
                  </p>
                )}
              </div>
